...
create or replace function public.has_pending_invitations(
  p_user_id uuid
)
returns boolean
language sql
...
revoke all on function public.has_pending_invitations(uuid) from public;
revoke all on function public.has_pending_invitations(uuid) from anon;
grant execute on function public.has_pending_invitations(uuid) to authenticated, service_role;

create or replace function public.list_received_invitations_with_cursor(
  p_limit integer default 10,
  p_cursor_created_time timestamp with time zone default null::timestamp with time zone,
  p_cursor_invitation_id uuid default null::uuid
)
returns table(
  invitation_id uuid,
  type text,
...
create or replace function public.respond_to_invitation_transactional(
  p_user_id uuid,
  ...
  if p_decision = 'accepted' then
    if v_inv.type = 'group' then
      ...
      if v_appointment_status = 'canceled' then
        ...
        set
          status = 'canceled',
          responded_time = coalesce(i.responded_time, now())
...
create or replace function public.update_appointment_status_transactional(
  p_user_id uuid,
