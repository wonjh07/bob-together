...
export default async function AppointmentDetailPage({
  ...
  const [result, commentsResult] = await Promise.all([
    ...
    getAppointmentCommentsAction(appointmentId),
  ]);
    ...
    return (
      <div className={styles.page}>
        ...
        <div className={styles.errorBox}>
          {errorMessage || '약속 정보를 불러올 수 없습니다.'}
        </div>
      </div>
    );
  }
  ...
  const initialComments =
    commentsResult.ok && commentsResult.data ? commentsResult.data.comments : [];
  const initialCommentCount =
    commentsResult.ok && commentsResult.data ? commentsResult.data.commentCount : 0;
